...
func DoneTask(ctx context.Context, req *task.DoneTaskRequest) (*model.Task, error) {
	var info *model.Task
	err := DB.Table(config.Service.Name).Where("id = ?", req.Id).First(&info).Error
	if err != nil {
		return nil, err
	}
	info.Status = DONE_FLAG
...
func UnDoneTask(ctx context.Context, req *task.UnDoneTaskRequest) (*model.Task, error) {
	var info *model.Task
	err := DB.Table(config.Service.Name).Where("id = ?", req.Id).First(&info).Error
	if err != nil {
		return nil, err
	}
	info.Status = UNDONE_FLAG
